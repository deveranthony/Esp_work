...
void app_main()
{
  ...
  ESP_ERROR_CHECK(nvs_flash_init_partition("nvs2"));

  nvs_handle handle;
  ESP_ERROR_CHECK(nvs_open_from_partition("nvs2", "cat_store", NVS_READWRITE, &handle));
  ...
  for (int i = 0; i < 5; i++)
  {
    ...
    esp_err_t result = nvs_get_blob(handle, catKey, (void *)&cat, &cat_size);
